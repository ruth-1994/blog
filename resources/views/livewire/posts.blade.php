<div class="container mx-auto mt-2">
    <x-banner/>
     <div class="flex content-center m-2 p-2">
         <x-button wire:click="showCreatePostModal" class="bg-green-500">Create Post</x-button>
     </div>
         ...
         <tbody class="bg-white divide-y divide-gray-200">
           ...
           <tr>
             ...
             <td class="px-6 py-4 text-right text-sm">
                <x-banner/>
               <x-button class="bg-white" wire:click="showEditPostModal({{ $post->id }})" class="bg-green-500">Edit</x-button>
               <x-button wire:click="deletePost({{ $post->id}})" class="bg-red-600">Delete</x-button>
             </td>
           </tr>
           @endforeach
           <!-- More items... -->
  ...
  <x-dialog-modal wire:model="showModalForm">
   ...
   <form enctype="multipart/form-data">
     <div class="sm:col-span-6">
       ...
       <div class="mt-1">
         <input type="text" id="title" wire:model.lazy="title" name="title" class="text-black block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
       </div>
     ...
     <div class="sm:col-span-6">
       ...
       <div class="mt-1">
         <input type="file" id="image" wire:model="image" name="image" class="text-black block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
       </div>
           @error('image') <span class="error text-red-600">{{ $message }}</span> @enderror
     </div>
     <div class="sm:col-span-6 pt-5">
       <label for="body" class="block text-sm font-medium text-white">Body</label>
       <div class="mt-1">
         <textarea id="body" rows="3" wire:model.lazy="body" class="text-black shadow-sm focus:ring-indigo-500 appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"></textarea>
       </div>
      ...
      <x-slot name="footer">
        @if($postId)
        <x-button wire:click="updatePost" class="dark:bg-yellow-400">Update</x-button>
        @else
        <x-button wire:click="storePost" class="dark:bg-yellow-400">store</x-button>
        @endif
      </x-slot>
  </x-dialog-modal>
 </div>
